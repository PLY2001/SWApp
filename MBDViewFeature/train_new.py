...
def main():
    target = 1
    input_weight = [0.0, 0.0, 0.0]
    input_weight[target] = 1.0
    ...
    data_transform = {
        "train": transforms.Compose([transforms.Resize(256),  # 图片等比例缩放，使得最小的边长为256
                                     transforms.RandomResizedCrop(224),
                                     #transforms.RandomHorizontalFlip(),
                                     transforms.ToTensor(),
                                     transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])]),
        "val": transforms.Compose([transforms.Resize(224),  # 图片等比例缩放，使得最小的边长为224
                                   transforms.ToTensor(),
                                   transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])}


    data_root = os.getcwd()  # get data root path
    image_path = os.path.join(data_root, "MBDViewDataset_noMBD/train")  # MBDViewDataset
    ...
    data_root = os.getcwd()  # get data root path
    test_image_path = os.path.join(data_root, "MBDViewDataset_noMBD/validation")  # flower data set path
    ...
    best_valloss = 100.0
    best_epoch = 0
    save_path = './MBDNet34_' + str(target) +'.pth'
    ...
    for epoch in range(epochs):
        ...
        for times in train_steps_bar:
            ...
            if (times + 1) % (batch_size*2 / batch_size) != 0:  # 每4次算一次
                continue
            loss = loss_function(outputslist[-2], outputslist[-1], 2, simWeight)
            outputslist.clear()
            loss.backward()
            optimizer.step()
            # print statistics
            ...
            # if (times + 1) % 2 == 0:
            #     train_loss_y_list.append(running_loss / (times + 1))
            #     train_loss_x_list.append(times / (train_steps - 1) + epoch)
            #     print(train_loss_x_list[-1], train_loss_y_list[-1], sep=',')
        ...
        with torch.no_grad():
            ...
            for times in val_steps_bar:
                ...
                loss = loss_function(valoutputslist[-2], valoutputslist[-1], 2, simWeight)
        ...
        print('[epoch %d] train_loss: %.3f  val_loss: %.3f' %
              (epoch + 1, running_loss / train_steps, valloss / val_steps))
