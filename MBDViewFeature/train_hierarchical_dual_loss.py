...
class HierarchicalTripletLoss(nn.Module):
    def __init__(self, margin_inter=1.0, margin_intra=0.1):
        ...
        # Use standard TripletMarginLoss for View A, which is a scalar output
        self.triplet_loss_A = nn.TripletMarginLoss(margin=margin_inter, p=2, reduction='mean')

        # Attention network to learn weights for the two loss components
    ...
    def forward(self, anchor_feats, positive_feats, negative_feats):
        anchor_feat_A, anchor_feat_B = anchor_feats[:, 0], anchor_feats[:, 1]
        positive_feat_A, positive_feat_B = positive_feats[:, 0], positive_feats[:, 1]
        negative_feat_A, _ = negative_feats[:, 0], negative_feats[:, 1] # Negative B is not used

        # --- 1. Inter-Class Loss (on View A for global features) ---
        # Standard triplet loss to push different classes apart based on shape.
        loss_inter = self.triplet_loss_A(anchor_feat_A, positive_feat_A, negative_feat_A)

        # --- 2. Intra-Class Loss (on View B for local details) ---
        # Goal: Maximize the distance (i.e., minimize similarity) between same-class items.
        # We want the similarity to be less than a negative margin.
        # The loss is relu(similarity + margin_intra).
        # If similarity = -0.2 and margin = 0.1, loss = relu(-0.1) = 0.
        # If similarity = 0.3 and margin = 0.1, loss = relu(0.4) = 0.4.
        # This encourages similarity to be < -margin_intra, preventing overfitting.
        similarity_intra = self.distance_func(anchor_feat_B, positive_feat_B)
        loss_intra_vec = F.relu(similarity_intra + self.margin_intra)

        # --- 3. Attention-based Weighted Sum ---
        # Use features from both views (detached to not affect their gradients through this path)
        # to decide the importance of each loss.
        attention_input = torch.cat([anchor_feat_A.detach(), anchor_feat_B.detach()], dim=1)
        weights = self.attention_net(attention_input)
        w_A, w_B = weights[:, 0], weights[:, 1] # Both are of size [batch_size]

        # Weight the losses. loss_inter is a scalar, loss_intra_vec is a vector.
        # We weight the intra-loss for each item in the batch, then take the mean.
        # The inter-loss is weighted by the mean of attention weights for the batch.
        total_loss = w_A.mean() * loss_inter + (w_B * loss_intra_vec).mean()
        
        return total_loss, loss_inter, loss_intra_vec.mean(), w_A.mean(), w_B.mean()
...
def main():
    ...
    # Initialize the new loss function with hyperparameters
    loss_function = HierarchicalTripletLoss(margin_inter=1.0, margin_intra=0.1).to(device)
    ...
    save_path_net = './MBDNet34_hierarchical_dual.pth'
    save_path_loss = './HierarchicalLoss_attention_dual.pth'
    ...
    for epoch in range(epochs):
        ...
        for anchor_views, positive_views, negative_views, _ in train_bar:
            ...
            loss, loss_inter, loss_intra, w_A, w_B = loss_function(anchor_feats, positive_feats, negative_feats)
            ...
            avg_w_A += w_A.item()
            avg_w_B += w_B.item()
            train_bar.set_description(f"Train E[{epoch+1}] L:{loss.item():.2f} (Inter:{loss_inter.item():.2f}, Intra:{loss_intra.item():.2f}) wA:{w_A:.2f} wB:{w_B:.2f}")
        ...
        print(f"[Epoch {epoch+1}] Train Loss: {train_loss:.4f} (Inter: {train_loss_inter:.4f}, Intra: {train_loss_intra:.4f}), Avg w_A: {avg_w_A:.3f}, Avg w_B: {avg_w_B:.3f}")
        ...
        with torch.no_grad():
            ...
            for anchor_views, positive_views, negative_views, _ in val_bar:
                ...
                loss, loss_inter, loss_intra, _, _ = loss_function(anchor_feats, positive_feats, negative_feats)
                val_loss += loss.item()
                val_loss_inter += loss_inter.item()
                val_loss_intra += loss_intra.item()
                val_bar.set_description(f"Val E[{epoch+1}] L:{loss.item():.2f}")
