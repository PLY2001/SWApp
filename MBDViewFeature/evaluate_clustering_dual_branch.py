...
def evaluate_coarse_grained_geometric(net_A, model_ids, device):
    ...
    with torch.no_grad():
        for model_info in tqdm(model_ids, desc="Processing models (Coarse Geo)"):
            ...
            final_feature = None
            # Load the first available view for type A (orthographic, front-cull) -> net_A
            for view_dir in range(6):
                ...
                if os.path.exists(img_path):
                    ...
                    final_feature = net_A(img_tensor).squeeze()
                    break  # Use the first view found
    ...
    return _run_clustering_and_evaluation(
        ...
        true_labels,
        coarse_labels,
        title_suffix="Coarse-Grained Geometric (net_A)",
        plot_filename="clustering_dual_coarse_geometric.png"
    )
...
def evaluate_fine_grained_geometric(net_B, model_ids, device):
    ...
    with torch.no_grad():
        for model_info in tqdm(model_ids, desc="Processing models (Fine Geo)"):
            ...
            final_feature = None
            # Load the first available view for type B (perspective, front-cull) -> net_B
            for view_dir in range(6):
                img_path = f"{base_path_model}_{view_dir}_1_0.bmp"
                if os.path.exists(img_path):
                    img = Image.open(img_path).convert('RGB')
                    img_tensor = data_transform(img).unsqueeze(0).to(device)
                    final_feature = net_B(img_tensor).squeeze()
                    break # Use the first view found
    ...
    return _run_clustering_and_evaluation(
        ...
        true_labels,
        coarse_labels,
        title_suffix="Fine-Grained Geometric (net_B)",
        plot_filename="clustering_dual_fine_geometric.png"
    )
...
def evaluate_multi_grained_geometric(net_A, net_B, model_ids, device):
    ...
    with torch.no_grad():
        for model_info in tqdm(model_ids, desc="Processing models (Multi Geo)"):
            ...
            feature_A, feature_B = None, None
            # Load the first available view for type A (orthographic) -> net_A
            for view_dir in range(6):
                img_path = f"{base_path_model}_{view_dir}_0_0.bmp"
                if os.path.exists(img_path):
                    img = Image.open(img_path).convert('RGB')
                    img_tensor = data_transform(img).unsqueeze(0).to(device)
                    feature_A = net_A(img_tensor).squeeze()
                    break
            
            # Load the first available view for type B (perspective) -> net_B
            for view_dir in range(6):
                img_path = f"{base_path_model}_{view_dir}_1_0.bmp"
                if os.path.exists(img_path):
                    img = Image.open(img_path).convert('RGB')
                    img_tensor = data_transform(img).unsqueeze(0).to(device)
                    feature_B = net_B(img_tensor).squeeze()
                    break
    ...
    return _run_clustering_and_evaluation(
        ...
        true_labels,
        coarse_labels,
        title_suffix="Multi-Grained Geometric (net_A + net_B)",
        plot_filename="clustering_dual_multi_geometric.png"
    )
...
def evaluate_coarse_grained_semantic(net_A, net_B, model_ids, device, w_C=0.3):
    ...
    with torch.no_grad():
        for model_info in tqdm(model_ids, desc="Processing models (Coarse Sem-Geo)"):
            ...
            feature_A, feature_B, feature_C = None, None, None
            
            # Geo-Coarse feature from net_A on the first ortho view
            for view_dir in range(6):
                img_path = f"{base_path_model}_{view_dir}_0_0.bmp"
                if os.path.exists(img_path):
                    img = Image.open(img_path).convert('RGB')
            ...
            # Geo-Fine feature from net_B on the first perspective view
            for view_dir in range(6):
                img_path = f"{base_path_model}_{view_dir}_1_0.bmp"
                if os.path.exists(img_path):
                    img = Image.open(img_path).convert('RGB')
            ...
            # Semantic feature from net_A on the first ortho, back-cull view
            for view_dir in range(6):
                img_path = f"{base_path_model}_{view_dir}_0_1.bmp"
                if os.path.exists(img_path):
                    img = Image.open(img_path).convert('RGB')
                    img_tensor = data_transform(img).unsqueeze(0).to(device)
                    feature_C = net_A(img_tensor).squeeze()
                    break

            if feature_A is None or feature_B is None or feature_C is None:
                continue
    ...
    return _run_clustering_and_evaluation(
        ...
        plot_filename=f"clustering_dual_coarse_semantic_wc_{w_C}.png"
    )
...
def plot_summary_results(results):
    ...
    ax.set_title('Comparison of Clustering Evaluation Metrics (Dual Branch Model)')
    ax.set_xticks(x)
    ...
    fig.tight_layout()
    plt.savefig('clustering_summary_dual_branch.png', dpi=300)
    print("  Summary plot saved to clustering_summary_dual_branch.png")
    plt.close()
...
def main():
    ...
    # --- Load Models ---
    print("Loading dual branch models...")
    net_A = resnet34().to(device)
    net_B = resnet34().to(device)
    
    path_A = "./MBDNet34_dual_A.pth"
    path_B = "./MBDNet34_dual_B.pth"
