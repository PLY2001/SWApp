...
def calculate_hierarchical_similarity(target_feats, db_feats, device):
    """
...
def main():
    ...
    # 3. Feature Database
    features_db = torch.from_numpy(np.load('features_hierarchical_sampling.npy'))
    with open('model_sampling_info.json', 'r') as f:
    ...
    # --- Perform Retrieval ---
    similarities = calculate_hierarchical_similarity(target_features, features_db, device)
