...
def _run_clustering_and_evaluation(all_features, true_labels, coarse_labels, title_suffix, plot_filename):
    ...
    fig, ax1 = plt.subplots(1, 1, figsize=(12, 10)) # New for single plot
    ...
    # 图1: 真实标签
    # 使用 nipy_spectral colormap，它能提供更多视觉上可区分的颜色
    ...
    for i, coarse_type in enumerate(unique_coarse_types):
        ...
        marker = markers[i % num_markers]  # 循环使用标记
    ...
    # # 图2: 聚类结果
    # colors_pred = plt.cm.get_cmap('nipy_spectral', n_clusters)
...
def evaluate_coarse_grained_geometric(net, model_ids, device):
    """
...
def evaluate_fine_grained_geometric(net, model_ids, device):
    """
...
def evaluate_multi_grained_geometric(net, model_ids, device):
    """
    验证任务3: 纯几何多粒度融合的聚类 (w_C = 0)
    """
    print("\n" + "=" * 50)
    print("  Running: Multi-Grained Geometric Clustering Evaluation")
    print("  Downsample Factor: 1x (No downsampling), Semantic Weight (wC): 0.0")
    print("=" * 50)
    ...
    return _run_clustering_and_evaluation(
        ...
        true_labels,
        coarse_labels,
        title_suffix="Multi-Grained Geometric (w_C=0)",
        plot_filename="clustering_multi_geometric.png"
    )


def evaluate_coarse_grained_semantic(net, model_ids, device, w_C=0.3):
    """
    验证任务4: 结合几何与语义的细粒度聚类
    """
    print("\n" + "=" * 50)
    print("  Running: Coarse-Grained Semantic-Enhanced Clustering Evaluation")
    print(f"  Downsample Factor: 1x, Semantic Weight (wC): {w_C}")
    print("=" * 50)
...
def main():
    ...
    # 1. Feature Extractor
    print("Loading feature extraction model (MBDNet34_hierarchical_dual.pth)...")
    net = resnet34().to(device)
    net.load_state_dict(torch.load("./MBDNet34_hierarchical_dual.pth", map_location=device))
    net.eval()
    ...
    # Task 1
    results['coarse_geo'] = evaluate_coarse_grained_geometric(net, model_ids, device)

    # Task 2
    results['fine_geo'] = evaluate_fine_grained_geometric(net, model_ids, device)

    # Task 3
    results['multi_geo'] = evaluate_multi_grained_geometric(net, model_ids, device)

    # Task 4
    results['coarse_sem'] = evaluate_coarse_grained_semantic(net, model_ids, device)
