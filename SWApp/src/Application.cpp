...
namespace MyApp {
	...
	CComPtr<ISurface> swSurface;//面
	VARIANT surfaceVT;//获取面的SAFEARRAY数组的载体
	...
	bool MyApplication::ReadMBD()
	{
		...
		if (hasMBDfeature && feData!=nullptr) {
			...
			for (int feIndex = 0; feIndex < featureCount; feIndex++) { //9991ms				
				...
				LPVOID ggaData = nullptr;//接收读取后的数组
				LONG ggannotationCount;//接收读取后的数组大小
				bool gghasAnno = true;
				if (!ReadSafeArray(&dimXpertAnnotationVT, VT_DISPATCH, 1, &ggaData, &ggannotationCount)) {
					//CoUninitialize();
					//return false;
					gghasAnno = false;//有时dimXpertAnnotationVT是空的
				}
				if (ggaData == nullptr) {
					gghasAnno = false;;//Realease模式下ReadSafeArray判断一直失误，不明原因
				}
				//gg
				if(gghasAnno)
				{
					...
					for (int fIndex = 0; fIndex < faceCount; fIndex++) { //15ms
						...
						double fArea;
						swFace->GetArea(&fArea);
						faceArea = fArea * 1000000 + faceArea;
						swEntity = swFace;
					...
					//c.读取面类型
					swSurface.Release();
					result = swFace->IGetSurface(&swSurface);
					if (result != S_OK) {
						CoUninitialize();
						return false;
					}
					LONG faceType;
					result = swSurface->Identity(&faceType);
					if (result != S_OK) {
						CoUninitialize();
						return false;
					}
					faceFeature.surfaceType = (swSurfaceTypes_e)faceType;
					...
					if (!ReadSafeArray(&dimXpertAnnotationVT, VT_DISPATCH, 1, &aData, &annotationCount)) {
						//CoUninitialize();
						//return false;
						hasAnno = false;//有时dimXpertAnnotationVT是空的
					}
					if (aData == nullptr) {
						hasAnno = false;;//Realease模式下ReadSafeArray判断一直失误，不明原因
					}
					...
					if (hasAnno)
					{
						...
						for (int aIndex = 0; aIndex < annotationCount; aIndex++) { //243ms
							...
							double dimSize = 0;
							ReadAnnotationData(dimXpertAnnotationType, &toleranceSize, &toleranceLevel, &datumName, toleranceDatumNames, &MCMType, &dimSize, faceArea);//可以返回公差等级（1.轴/孔 2.线性尺寸 3.形位公差 三者等级定义不同）

							//d.存入特征面数组的标注数组
							MyAnnotation myAnnotation;
							myAnnotation.Name = dimXpertAnnotationNameStr;
							myAnnotation.NameBSTR = dimXpertAnnotationName;
							myAnnotation.Type = dimXpertAnnotationType;
							myAnnotation.IsGeoTolerance = GeoTolMap[dimXpertAnnotationType] > 0 ? GeoTolMap[dimXpertAnnotationType] : 0;
							myAnnotation.IsDimTolerance = DimTolMap[dimXpertAnnotationType] > 0 ? DimTolMap[dimXpertAnnotationType] : 0;
							...
							myAnnotation.hasMCM = MCMMap[MCMType] > 0 ? MCMMap[MCMType] : 0;
					...
					if (entityType == swSelectType_e::swSelFACES) {//实体是否是面
						...
						for (int eIndex = 0; eIndex < eCount; eIndex++) {
							...
							if (FaceMap.count(faceName) > 0) {
								...
								SFSAnnotation.IsSFSymbol = SFSMap[(swSFSymType_e)SFSymbolType];
								SFSAnnotation.Name = "粗糙度" + textstr;
								SFSAnnotation.AccuracySize = SFSymbolSize;
								SetSFSAccuarcyLevel(&SFSAnnotation.AccuracyLevel, SFSymbolSize);
			...
			for (int bIndex = 0; bIndex < bCount; bIndex++) { //1294ms
				...
				while (thisFace)
				{
					...
					if (faceName == "") { //名称为空说明该面没有读取过特征标注
						noMBDFaceCount++;
						VARIANT_BOOL isSelected;
						result = swEntity->Select4(VARIANT_FALSE, nullptr, &isSelected);
						//hasNoMBDFace = true;
						...
						//读取面类型
						swSurface.Release();
						result = thisFace->IGetSurface(&swSurface);
						if (result != S_OK) {
							CoUninitialize();
							return false;
						}
						...
						noMBDFace.surfaceType = (swSurfaceTypes_e)faceType;
						noMBDFace.AnnotationCount = 0;
						noMBDFace.Name = "0_NoMBDFace_"+ std::to_string(noMBDFaceCount);
						...
						FaceMap[noMBDFace.Name] = noMBDFace;

						long error = NOERROR;
						long warning = NOERROR;
						VARIANT_BOOL isSaved;
						CComBSTR savePath = (CADTempPath + CADName + "\\" + noMBDFace.Name + ".STL").c_str();
						result = swDoc->SaveAs4(savePath, swSaveAsCurrentVersion, swSaveAsOptions_Silent, &error, &warning, &isSaved);//保存文件
						if (result != S_OK) {
							CoUninitialize();
							return false;
...
// 			}
	...
	void MyApplication::ReadAnnotationData(swDimXpertAnnotationType_e annoType, double* toleranceSize, int* toleranceLevel, std::string* myDatumName, std::vector<std::string>& datumNames, swDimXpertMaterialConditionModifier_e* MCMType, double* dimSize, double faceArea)
	{
		if (annoType == swDimXpertAnnotationType_e::swDimXpertDatum) {
			DatumData(myDatumName);
		}
		else if (GeoTolMap[annoType]>0) {
			GeoTolData(annoType, toleranceSize, toleranceLevel, datumNames, MCMType, faceArea);
		}
	...
	void MyApplication::GeoTolData(swDimXpertAnnotationType_e annoType, double* toleranceSize, int* toleranceLevel, std::vector<std::string>& datumNames, swDimXpertMaterialConditionModifier_e* MCMType, double faceArea)
	{
		...
		SetGeoAccuarcyLevel(toleranceLevel, *toleranceSize, faceArea, annoType);//用国标判断,由面的尺寸得公差等级
	...
	void MyApplication::DimTolData(swDimXpertAnnotationType_e annoType, double* toleranceSize, int* toleranceLevel, double* dimSize)
	{
		...
		*dimSize = nominalValue;
		...
		upper *= dbl;
		lower *= dbl;
		double tolSize = upper - lower;
		*toleranceSize = tolSize;

		//用国标判断公差等级
		swDimXpertDimensionToleranceType_e DimensionToleranceType;
		dimensionTolerance->get_DimensionType(&DimensionToleranceType);//swDimXpertDimTolType_ISOLimitsAndFitsNoNominal
		if (DimensionToleranceType == swDimXpertDimTolType_ISOLimitsAndFitsNoNominal) //孔轴尺寸公差
		{
			CComBSTR fitTolCode;
			dimensionTolerance->get_LimitsAndFitsCode(&fitTolCode);//g6
			*toleranceLevel = FitTolMap[GbkToUtf8(_com_util::ConvertBSTRToString(fitTolCode))];

		}
		else //线性尺寸公差
		{
			if (nominalValue <= 30.0) {
				if (nominalValue > 6.0) {
					SetLinearDimAccuarcyLevel(toleranceLevel, tolSize, 0.1, 0.2, 0.5, 1.0);
				}
				else if (nominalValue > 3.0) {
					SetLinearDimAccuarcyLevel(toleranceLevel, tolSize, 0.05, 0.1, 0.3, 0.5);
				}
				else {
					SetLinearDimAccuarcyLevel(toleranceLevel, tolSize, 0.05, 0.1, 0.2, 0.5);
				}
			}
			else
			{
				if (nominalValue <= 120.0) {
					SetLinearDimAccuarcyLevel(toleranceLevel, tolSize, 0.15, 0.3, 0.8, 1.5);
				}
				else if (nominalValue <= 400.0) {
					SetLinearDimAccuarcyLevel(toleranceLevel, tolSize, 0.2, 0.5, 1.2, 2.5);
				}
				else if (nominalValue <= 1000.0) {
					SetLinearDimAccuarcyLevel(toleranceLevel, tolSize, 0.3, 0.8, 2.0, 4.0);
				}
				else if (nominalValue <= 2000.0) {
					SetLinearDimAccuarcyLevel(toleranceLevel, tolSize, 0.5, 1.2, 3.0, 6.0);
				}
			}
			...
			do
			{
				...
				if (fileinfo.attrib & _A_SUBDIR) {
					if (strcmp(fileinfo.name, ".") != 0 && strcmp(fileinfo.name, "..") != 0) {
						ClearFiles(clearPath + fileinfo.name + "\\");//path要加上子目录名
					}
				}
				//如果不是目录或者隐藏文件
				if (!(fileinfo.attrib & _A_SUBDIR || fileinfo.attrib & _A_HIDDEN))
	...
	void MyApplication::SetLinearDimAccuarcyLevel(int* level, double size, float f, float m, float c, float v)
	{
		if (size <= f*2.0) {
			*level = 1;
		}
		else if (size > f*2.0 && size <= m*2.0) {
			*level = 3;
		}
		else if (size > m*2.0 && size <= c*2.0) {
			*level = 5;
		}
		else if (size > c*2.0) {
			*level = 7;
	...
	void MyApplication::SetSFSAccuarcyLevel(int* level, double size)
	{
		if (size <= 3.2) {
			if (size > 1.6) {
				*level = 5;
...
#define PI 3.1415926
	void MyApplication::SetGeoAccuarcyLevel(int* level, double size, double faceArea, swDimXpertAnnotationType_e annoType)
	{
		double d = sqrt(faceArea/PI);
		if (annoType == swDimXpertGeoTol_Concentricity || annoType == swDimXpertGeoTol_Symmetry || annoType == swDimXpertGeoTol_CircularRunout || annoType == swDimXpertGeoTol_TotalRunout) {
			if (d <= 30.0) {
				if (d > 18.0) {
					SetGeoRealAccuarcyLevel(level, size, 6, 10, 15, 25, 50, 100);
				}
				else if (d > 10.0) {
					SetGeoRealAccuarcyLevel(level, size, 5, 8, 12, 20, 40, 80);
				}
				else if (d > 6.0) {
					SetGeoRealAccuarcyLevel(level, size, 4, 6, 10, 15, 30, 60);
				}
				else {
					SetGeoRealAccuarcyLevel(level, size, 3, 5, 8, 12, 25, 50);
				}
			}
			else {
				if (d <= 50.0) {
					SetGeoRealAccuarcyLevel(level, size, 8, 12, 20, 30, 60, 120);
				}
				else if (d <= 120.0) {
					SetGeoRealAccuarcyLevel(level, size, 10, 15, 25, 40, 80, 150);
				}
				else if (d <= 250.0) {
					SetGeoRealAccuarcyLevel(level, size, 12, 20, 30, 50, 100, 200);
				}
				else {
					SetGeoRealAccuarcyLevel(level, size, 15, 25, 40, 60, 120, 250);
				}
			}
		}
		else if (annoType == swDimXpertGeoTol_Parallelism || annoType == swDimXpertGeoTol_Perpendicularity || annoType == swDimXpertGeoTol_Angularity) {
			if (d <= 40.0) {
				if (d > 25.0) {
					SetGeoRealAccuarcyLevel(level, size, 10, 15, 25, 40, 60, 100);
				}
				else if (d > 16.0) {
					SetGeoRealAccuarcyLevel(level, size, 8, 12, 20, 30, 50, 80);
				}
				else if (d > 10.0) {
					SetGeoRealAccuarcyLevel(level, size, 6, 10, 15, 25, 40, 60);
				}
				else {
					SetGeoRealAccuarcyLevel(level, size, 5, 8, 12, 20, 30, 50);
				}
			}
			else {
				if (d <= 63.0) {
					SetGeoRealAccuarcyLevel(level, size, 12, 20, 30, 50, 80, 120);
				}
				else if (d <= 100.0) {
					SetGeoRealAccuarcyLevel(level, size, 15, 25, 40, 60, 100, 150);
				}
				else if (d <= 160.0) {
					SetGeoRealAccuarcyLevel(level, size, 20, 30, 50, 80, 120, 200);
				}
				else if (d <= 250.0) {
					SetGeoRealAccuarcyLevel(level, size, 25, 40, 60, 100, 150, 250);
				}
				else if (d <= 400.0) {
					SetGeoRealAccuarcyLevel(level, size, 30, 50, 80, 120, 200, 300);
				}
				else {
					SetGeoRealAccuarcyLevel(level, size, 40, 60, 100, 150, 250, 400);
				}
			}
		}
		else if (annoType == swDimXpertGeoTol_Straightness || annoType == swDimXpertGeoTol_Flatness) {
			if (d <= 40.0) {
				if (d > 25.0) {
					SetGeoRealAccuarcyLevel(level, size, 4, 6, 10, 15, 25, 40);
				}
				else if (d > 16.0) {
					SetGeoRealAccuarcyLevel(level, size, 3, 5, 8, 12, 20, 30);
				}
				else if (d > 10.0) {
					SetGeoRealAccuarcyLevel(level, size, 2.5, 4, 6, 10, 15, 25);
				}
				else {
					SetGeoRealAccuarcyLevel(level, size, 2, 3, 5, 8, 12, 20);
				}
			}
			else {
				if (d <= 63.0) {
		...
		else if (annoType == swDimXpertGeoTol_Circularity || annoType == swDimXpertGeoTol_Cylindricity) {
			if (d <= 50.0) {
				if (d > 30.0) {
					SetGeoRealAccuarcyLevel(level, size, 2.5, 4, 7, 11, 16, 25);
				}
				else if (d > 18.0) {
					SetGeoRealAccuarcyLevel(level, size, 2.5, 4, 6, 9, 13, 21);
				}
				else if (d > 10.0) {
					SetGeoRealAccuarcyLevel(level, size, 2, 3, 5, 8, 11, 18);
				}
				else {
					SetGeoRealAccuarcyLevel(level, size, 1.5, 2.5, 4, 6, 9, 15);
				}
			}
			else {
				if (d <= 80.0) {
					SetGeoRealAccuarcyLevel(level, size, 3, 5, 8, 13, 19, 30);
				}
				else if (d <= 120.0) {
					SetGeoRealAccuarcyLevel(level, size, 4, 6, 10, 15, 22, 35);
				}
				else if (d <= 180.0) {
					SetGeoRealAccuarcyLevel(level, size, 5, 8, 12, 18, 25, 40);
				}
				else if (d <= 250.0) {
					SetGeoRealAccuarcyLevel(level, size, 7, 10, 14, 20, 29, 46);
				}
				else if (d <= 315.0) {
					SetGeoRealAccuarcyLevel(level, size, 8, 12, 16, 23, 32, 52);
				}
				else if (d <= 400.0) {
					SetGeoRealAccuarcyLevel(level, size, 9, 13, 18, 25, 36, 57);
				}
				else {
					SetGeoRealAccuarcyLevel(level, size, 10, 15, 20, 27, 40, 63);
				}
			}
		}
	...
	void MyApplication::SetGeoRealAccuarcyLevel(int* level, double size, float a, float b, float c, float d, float e, float f)
	{
		double s = size * 1000;
		if (s < a) {
			*level = 1;
